"""
Rates within-complex analytics.

Sub-components: 10Y yield, 2s10s spread, 10Y breakeven, 10Y real, MOVE.

Asset conventions
-----------------
All five sub-components are diff()'d (first differences):
- USGG10YR  : 10Y yield level → daily change in pct points
- USYC2Y10  : 2s10s spread → daily change in pct points
- USGGBE10  : 10Y breakeven → daily change in pct points
- USGGT10Y  : 10Y real yield → daily change in pct points
- MOVE      : implied rates vol level → daily change in vol points

Within-window z-scoring puts everything on a common footing despite the
mixed units (yield bp diffs vs vol-point diffs).

Sign anchor
-----------
PC1 is anchored so USGG10YR_load is positive each day. So "positive PC1"
always means "rates higher" — the macro-natural direction for a
rates-internal model.
"""
...
# ---------------------------------------------------------------------------
# Asset list — single source of truth
# ---------------------------------------------------------------------------
ASSETS = ["USGG10YR", "USYC2Y10", "USGGBE10", "USGGT10Y", "MOVE"]
...
def compute_returns(
    ...
    """
    All 5 sub-components are first-differenced.
    """
...
def all_pair_keys() -> list[str]:
    """All 10 unique pairs in canonical order."""
    keys = []
...
def pca_dominant_theme(
    ...
    """PCA on the LAST `window` days, anchored on USGG10YR positive."""
...
def rolling_pca_loadings(
    ...
    """
    Rolling PC1 loadings. Anchored on USGG10YR positive (per-day or procrustes).
    """
...
def leadership_stats(loadings_df: pd.DataFrame) -> pd.DataFrame:
    """
    Per-day: which sub-component leads, how concentrated, sign pattern.
    """
...
def headline_vs_breadth(returns: pd.DataFrame, window: int = 5) -> pd.DataFrame:
    """
    Compare USGG10YR's recent move to the median absolute move across all 5
    rates sub-components, on a vol-scaled basis. Flags when the headline (10Y)
    is doing more / less work than the broader complex.

    Returns DataFrame indexed by Date with columns:
        HeadlineMove   : trailing-window cumulative |move| for USGG10YR, vol-scaled
        ComplexMedian  : median across 5 sub-components of the same metric
        Divergence     : HeadlineMove - ComplexMedian
                         (positive = 10Y is leading; negative = 10Y is lagging)
    """
    if returns.empty:
        return pd.DataFrame(columns=["HeadlineMove", "ComplexMedian", "Divergence"])

    # Standardize each column by its own long-history vol so cross-asset units
    # align (yields, BE, real, MOVE all in different scales).
    rv = returns.rolling(window=60, min_periods=20).std()
    scaled = (returns / rv).abs()

    # Trailing window cumulative |move|
...
__ANALYTICS_VERSION__ = "rates-v1.0-2026-05-09"
