"""
Credit within-complex analytics.

Sub-components: LF98OAS (US HY), LUACOAS (US IG), ITRXEBE (EU iTraxx).

Asset conventions
-----------------
All three sub-components are first-differenced. They're all spread series
(in bp / OAS units), so daily change in pct points is the natural "return".

Sign anchor
-----------
PC1 is anchored so LF98OAS_load is positive each day. So "positive PC1"
always means "HY wider" — the macro-natural risk-off direction for a
credit-internal model.

Structural notes
----------------
- 3 sub-components is the lower bound of useful PCA. PC1 will explain
  a higher share of variance than 5-asset baskets (typically 60-85% in
  credit). Concentration thresholds adjusted accordingly.
- HY-IG ratio is intentionally NOT in the PCA — it's a linear combination
  of HY and IG, so adding it would create exact collinearity. It's
  surfaced as a side-panel diagnostic instead.
"""
...
# Asset list
# ---------------------------------------------------------------------------
ASSETS = ["LF98OAS", "LUACOAS", "ITRXEBE"]
...
def compute_returns(prices: pd.DataFrame, vol_scale: bool = False,
    ...
    """All 3 sub-components are first-differenced (spread units)."""
...
# ---------------------------------------------------------------------------
# Pairwise rolling correlations (3 pairs)
# ---------------------------------------------------------------------------
...
def all_pair_keys() -> list[str]:
    """All 3 unique pairs in canonical order."""
    keys = []
...
def pca_dominant_theme(returns: pd.DataFrame, window: int = 60,
                       weighting: str = "equal") -> dict:
    """PCA on the LAST `window` days, anchored on LF98OAS positive."""
...
def rolling_pca_loadings(returns: pd.DataFrame, window: int = 60,
    ...
    """Rolling PC1 loadings. Anchored on LF98OAS positive."""
...
def headline_vs_breadth(returns: pd.DataFrame, window: int = 5) -> pd.DataFrame:
    """Compare LF98OAS recent move to median across the credit complex."""
    if returns.empty:
...
# ---------------------------------------------------------------------------
# HY-IG ratio (quality dispersion diagnostic)
# ---------------------------------------------------------------------------
def hy_ig_ratio(prices: pd.DataFrame) -> pd.Series:
    """
    HY OAS / IG OAS. When the ratio rises, HY is widening *more* than just
    beta to IG — a quality dispersion signal. Falls during compression
    rallies. Standard credit-shape indicator analogous to 2s10s in rates.
    """
    if "LF98OAS" not in prices.columns or "LUACOAS" not in prices.columns:
        return pd.Series(dtype=float, name="HY_IG_Ratio")
    ratio = prices["LF98OAS"] / prices["LUACOAS"]
    return ratio.rename("HY_IG_Ratio")


# ---------------------------------------------------------------------------
# US-EU divergence (cross-region credit signal)
# ---------------------------------------------------------------------------
def us_eu_divergence(returns: pd.DataFrame, window: int = 20) -> pd.Series:
    """
    Rolling correlation between US HY moves and EU iTraxx moves.
    When this drops, US/EU credit are decoupling — sometimes a regime signal
    (idiosyncratic stress on one side, e.g. EU sovereign episodes).
    """
    if returns.empty or "LF98OAS" not in returns.columns or "ITRXEBE" not in returns.columns:
        return pd.Series(dtype=float, name="US_EU_Corr")
    return returns["LF98OAS"].rolling(window).corr(returns["ITRXEBE"]).rename("US_EU_Corr")
...
def concentration_label(c: float) -> str:
    """
    For 3 assets, concentration floor is 1/3 = 0.333. Adjusted thresholds:
    """
    if c < 0.40:   return "diffuse"      # close to 0.333 floor
    elif c < 0.50: return "mild lead"
    elif c < 0.65: return "clear lead"
    else:          return "dominant"


__ANALYTICS_VERSION__ = "credit-v1.0-2026-05-09"
