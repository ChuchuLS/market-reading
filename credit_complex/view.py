...
def load_prices(path: Path, _mtime: float) -> pd.DataFrame:
    """Read FICCREADING.xlsx and extract credit sub-components."""
    ...
    if missing:
        raise ValueError(
            f"FICCREADING.xlsx is missing credit sub-components: {missing}. "
...
def render_credit_complex():
    ...
    try:
        ...
        st.error(f"Failed to read FICCREADING.xlsx for credit: {e}")
        return
