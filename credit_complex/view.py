"""
Credit complex view — within-complex PCA on 3 credit sub-components.

Sub-components: LF98OAS (US HY), LUACOAS (US IG), ITRXEBE (EU iTraxx).
Reads the same MARKET_DATA.xlsx (sheet: ficc) as cross_asset_ficc/.

Sub-tabs:
  1. Heatmap — grayscale + sign symbol overlay (3 rows)
  2. Drill-down small multiples — blue/orange fill by sign-vs-HY
  3. Correlations & Theme — 3 pairwise + leadership decomposition
  4. Regime — continuous regime label + transitions

Plus a "Headline vs breadth" diagnostic strip and a credit-shape side panel
(HY-IG ratio + US-EU correlation).
"""
...
def load_prices(path: Path, _mtime: float) -> pd.DataFrame:
    """Read MARKET_DATA.xlsx (sheet: ficc) and extract credit sub-components."""
    ...
    if missing:
        raise ValueError(
            f"MARKET_DATA.xlsx (sheet: ficc) is missing credit sub-components: {missing}. "
            f"Required: LF98OAS, LUACOAS, ITRXEBE. "
...
# Public render
# ===========================================================================
def render_credit_complex():
    if not DATA_PATH.exists():
        st.error(
            f"MARKET_DATA.xlsx (sheet: ficc) not found at {DATA_PATH}.\n\n"
            "Required columns: **LF98OAS, LUACOAS, ITRXEBE**."
        )
        return
    ...
    try:
        ...
        st.error(f"Failed to read MARKET_DATA.xlsx (sheet: ficc) for credit: {e}")
        return
    ...
    # Methodology controls (credit-specific session keys)
    st.markdown(
        """
    ...
    with pc1:
        if st.button("⚡ Standard", use_container_width=True, key="credit_preset_std"):
            for ns in ("credit_corr", "credit_pca"):
                st.session_state[f"{ns}_window"] = 20
                st.session_state[f"{ns}_weighting"] = "equal"
            st.session_state["credit_scaling"] = "zscore"
            st.session_state["credit_pca_method"] = "standard"
            st.session_state["credit_pca_presmooth"] = 0
            st.rerun()
    with pc2:
        if st.button("〜 Smooth", use_container_width=True, key="credit_preset_smooth"):
            for ns in ("credit_corr", "credit_pca"):
                st.session_state[f"{ns}_window"] = 90
                st.session_state[f"{ns}_weighting"] = "equal"
            st.session_state["credit_scaling"] = "zscore"
            st.session_state["credit_pca_method"] = "procrustes"
    ...
    with pc3:
        if st.button(
            ...
            st.session_state["credit_scaling"] = "zscore"
            st.session_state["credit_pca_method"] = "procrustes"
            st.session_state["credit_pca_presmooth"] = 20
            st.rerun()
    ...
    settings_html = f"""
        <div style="background:#0f0f0f;border:1px solid #2a2a2a;border-left:3px solid #fbbf24;
          ...
          <div><span style="color:#fbbf24;font-weight:600;">SHARED:</span>
            scaling={scaling} · anchor=LF98OAS(+)</div>
        </div>
    """
    ...
    # ---- Credit shape side panel (HY/IG ratio + US-EU correlation) ---------
    _render_credit_shape_panel(prices, returns)
...
# Credit shape side panel — HY/IG ratio + US-EU correlation
# ---------------------------------------------------------------------------
def _render_credit_shape_panel(prices: pd.DataFrame, returns: pd.DataFrame):
    """
    Two diagnostics side by side:
    - HY/IG ratio (quality dispersion within US credit)
    - US-EU rolling correlation (regional cohesion)
    """
    ratio = hy_ig_ratio(prices)
    us_eu = us_eu_divergence(returns, window=20)
    ...
    # HY/IG regime label
    if last_ratio > 4.5:
        ratio_label = "STRESSED"
        ratio_color = "#f97316"
    elif last_ratio > 3.5:
        ratio_label = "ELEVATED"
        ratio_color = "#facc15"
    elif last_ratio > 2.5:
        ratio_label = "NORMAL"
        ratio_color = "#3b82f6"
    else:
        ratio_label = "COMPRESSED"
        ratio_color = "#06b6d4"

    sign5 = "+" if last_5d_change >= 0 else ""
    sign20 = "+" if last_20d_change >= 0 else ""

    # US-EU correlation reading
    if not us_eu.empty:
        last_corr = float(us_eu.iloc[-1])
        if last_corr > 0.7:
            corr_label = "ALIGNED"
            corr_color = "#3b82f6"
        elif last_corr > 0.4:
            ...
            corr_label = "DIVERGING"
            corr_color = "#facc15"
        else:
            corr_label = "DECOUPLED"
            corr_color = "#f97316"
    else:
        last_corr = np.nan
        corr_label = ""
        ...
        st.markdown(
            ...
            <div style="text-align:left;">
              <div style="font-size:10px;color:#888;letter-spacing:0.08em;
                          text-transform:uppercase;">HY/IG RATIO</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:24px;
                          font-weight:700;color:#fff;">{last_ratio:.2f}x</div>
              <div style="font-size:11px;color:{ratio_color};font-weight:600;
                          letter-spacing:0.05em;">{ratio_label}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            <div style="text-align:left;">
              ...
              <div style="font-family:'JetBrains Mono',monospace;font-size:18px;
                          font-weight:700;color:#fff;">{sign5}{last_5d_change:.2f}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with cols[2]:
        st.markdown(
            ...
            <div style="text-align:left;">
              ...
              <div style="font-family:'JetBrains Mono',monospace;font-size:18px;
                          font-weight:700;color:#fff;">{sign20}{last_20d_change:.2f}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with cols[3]:
        if not np.isnan(last_corr):
            st.markdown(
                f"""
                <div style="text-align:left;">
                  <div style="font-size:10px;color:#888;letter-spacing:0.08em;
                              text-transform:uppercase;">US-EU CORR (20D)</div>
                  <div style="font-family:'JetBrains Mono',monospace;font-size:24px;
                              font-weight:700;color:#fff;">{last_corr:+.2f}</div>
                  <div style="font-size:11px;color:{corr_color};font-weight:600;
                              letter-spacing:0.05em;">{corr_label}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
def _render_heatmap_panel(loadings: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
          ...
          3 rows, one per credit sub-component. Cell <b>brightness</b> = how much
          that sub-component is participating in the dominant theme.
          <b>+</b> = same direction as US HY OAS, <b>−</b> = opposite.
          With 3 components, the regime is either coherent (all +) or
          breaking (one −) — there's no intermediate state to hide in.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.caption(
        ...
        "(typically 60-85% with only 3 components). "
...
def _render_drilldown_panel(loadings: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
          ...
          <span style="color:#3b82f6;font-weight:600;">Blue</span> = same side as HY OAS,
    ...
    st.caption(
        ...
        "Blue fill = component same side of PC1 as HY · "
        "Orange fill = opposite."
    )


# ---------------------------------------------------------------------------
# Tab 3a: Pairwise correlations (3 pairs)
# ---------------------------------------------------------------------------
def _render_correlations_panel(returns: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
                    ...
                    margin-bottom:1rem;">
          <span style="color:#fbbf24;font-weight:600;">What this shows:</span>
          The 3 unique pairs in the credit complex. Cell <b>brightness</b> = |ρ|;
...
def _render_dominant_theme_panel(returns: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
                    ...
                    margin-bottom:1rem;">
          <span style="color:#fbbf24;font-weight:600;">What this shows:</span>
          PC1 loadings for all 3 credit sub-components, the leader (largest |loading|),
    ...
    with pc2:
        presmooth_halflife = st.select_slider(
            ...
            value=st.session_state.get("credit_pca_presmooth", 0),
            key="credit_pca_presmooth",
        )
    ...
    st.caption(
        f"Same sign = moving with HY direction · Opposite sign = diverging · "
...
def _render_regime_panel(loadings: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
                    ...
                    margin-bottom:1rem;">
          <span style="color:#fbbf24;font-weight:600;">What this shows:</span>
          A regime is identified by <b>(leader, 3-bit sign pattern)</b>.
          Days where the dominant theme is weak (PC1 ExpVar &lt; {var:.0%},
