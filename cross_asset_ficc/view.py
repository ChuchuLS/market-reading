...
def _render_dominant_theme_panel(loadings: pd.DataFrame, returns: pd.DataFrame):
    ...
    # The sliders above write to session_state (keys ficc_pca_*). The render
    # function reads those same keys to compute `loadings`, which is passed in
    # here — so the headline, chart, and regime all share ONE PCA computation.
    # (Changing a slider triggers a rerun; the new settings take effect then.)
    roll = loadings
    latest = roll.iloc[-1]
