...
def load_prices(path: Path, _mtime: float) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            f"FICCREADING.xlsx is missing rates sub-components: {missing}. "
...
def render_rates_complex():
    ...
    try:
        ...
        st.error(f"Failed to read FICCREADING.xlsx for rates: {e}")
        return
    ...
    with pc2:
        if st.button("〜 Smooth", use_container_width=True, key="rates_preset_smooth"):
            ...
            st.rerun()
    with pc3:
        if st.button("≈ Very Smooth", use_container_width=True, key="rates_preset_vsmooth"):
            for ns in ("rates_corr", "rates_pca"):
...
def _render_headline_diagnostic(returns: pd.DataFrame, prices: pd.DataFrame):
    ...
    if abs(div) < 0.5:
        ...
        msg = (f"10Y is moving <b style='color:#fff;'>more</b> than the rest of the "
               f"complex (divergence +{div:.2f}σ over 5d). The headline is leading.")
        msg_color = "#3b82f6"
    else:
        msg = (f"10Y is moving <b style='color:#fff;'>less</b> than the rest of the "
...
def _render_curve_panel(prices: pd.DataFrame):
    ...
    with cols[3]:
        ...
        fig.add_hline(y=0, line=dict(color="rgba(249,115,22,0.4)", width=1, dash="dash"))
        fig.update_layout(
            **{**DARK_LAYOUT,
...
def _render_dominant_theme_panel(returns: pd.DataFrame):
    ...
    with pc4:
        pca_method = st.radio(
            ...
            format_func=lambda x: {"standard": "Per-day 10Y+",
