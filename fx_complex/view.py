...
def load_prices(path: Path, _mtime: float) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            f"MARKET_DATA.xlsx (sheet: ficc) is missing FX sub-components: {missing}. "
...
def render_fx_complex():
    ...
    try:
        ...
        st.error(f"Failed to read MARKET_DATA.xlsx (sheet: ficc) for FX: {e}")
        return
    ...
    with pc3:
        if st.button(
            "≈ Very Smooth", use_container_width=True, key="fx_preset_vsmooth"
        ):
            for ns in ("fx_corr", "fx_pca"):
...
def _render_headline_diagnostic(returns: pd.DataFrame):
    ...
    if abs(div) < 0.5:
        ...
        msg = (
            f"DXY is moving <b style='color:#fff;'>more</b> than the rest of "
            f"the FX complex (divergence +{div:.2f}σ over 5d). DXY-led regime."
        )
        msg_color = "#3b82f6"
    else:
        msg = (
            f"DXY is moving <b style='color:#fff;'>less</b> than the rest of "
