"""
FX complex view — within-complex PCA on 3 FX sub-components.
...
from fx_complex.analytics import (
...
def load_prices(path: Path, _mtime: float) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            f"FICCREADING.xlsx is missing FX sub-components: {missing}. "
            f"Required: DXY, FXJPEMCS, JYBSS12M. "
...
# Public render
# ===========================================================================
def render_fx_complex():
    if not DATA_PATH.exists():
        st.error(
            f"FICCREADING.xlsx not found at {DATA_PATH}.\n\n"
            "Required columns: **DXY, FXJPEMCS, JYBSS12M**."
        )
        return
    ...
    try:
        ...
        st.error(f"Failed to read FICCREADING.xlsx for FX: {e}")
        return
    ...
    with pc1:
        if st.button("⚡ Standard", use_container_width=True, key="fx_preset_std"):
            for ns in ("fx_corr", "fx_pca"):
                st.session_state[f"{ns}_window"] = 20
                st.session_state[f"{ns}_weighting"] = "equal"
            st.session_state["fx_scaling"] = "zscore"
            st.session_state["fx_pca_method"] = "standard"
            st.session_state["fx_pca_presmooth"] = 0
            st.rerun()
    with pc2:
        if st.button("〜 Smooth", use_container_width=True, key="fx_preset_smooth"):
            for ns in ("fx_corr", "fx_pca"):
                st.session_state[f"{ns}_window"] = 90
                st.session_state[f"{ns}_weighting"] = "equal"
            st.session_state["fx_scaling"] = "zscore"
            st.session_state["fx_pca_method"] = "procrustes"
            st.session_state["fx_pca_presmooth"] = 15
            st.rerun()
    with pc3:
        if st.button("≈ Very Smooth", use_container_width=True, key="fx_preset_vsmooth"):
            for ns in ("fx_corr", "fx_pca"):
                st.session_state[f"{ns}_window"] = 120
                st.session_state[f"{ns}_weighting"] = "ewm"
            st.session_state["fx_scaling"] = "zscore"
            st.session_state["fx_pca_method"] = "procrustes"
            st.session_state["fx_pca_presmooth"] = 20
            st.rerun()
    ...
    with tbc1:
        scaling = st.radio(
            ...
            index=["zscore", "volscale"].index(
                st.session_state.get("fx_scaling", "zscore")),
    ...
    with tbc2:
        st.markdown("<div style='font-size:10px;color:transparent;'>spacer</div>",
                    unsafe_allow_html=True)
        if st.button("↻ Refresh data", use_container_width=True, key="fx_refresh"):
            st.cache_data.clear()
            st.rerun()

    corr_window      = st.session_state.get("fx_corr_window", 20)
    corr_weighting   = st.session_state.get("fx_corr_weighting", "equal")
    pca_window       = st.session_state.get("fx_pca_window", 20)
    pca_weighting    = st.session_state.get("fx_pca_weighting", "equal")
    pca_method       = st.session_state.get("fx_pca_method", "standard")
    pca_presmooth    = st.session_state.get("fx_pca_presmooth", 0)
    ...
    settings_html = f"""
        <div style="background:#0f0f0f;border:1px solid #2a2a2a;border-left:3px solid #fbbf24;
          ...
          <div><span style="color:#fbbf24;font-weight:600;">SHARED:</span>
            scaling={scaling} · anchor=DXY(+)</div>
        </div>
    """
    ...
    _render_funding_stress_panel(prices)
...
def _render_headline_diagnostic(returns: pd.DataFrame):
    ...
    if abs(div) < 0.5:
        msg = "DXY is moving in line with the rest of the FX complex."
        msg_color = "#888"
    elif div > 0:
        msg = (f"DXY is moving <b style='color:#fff;'>more</b> than the rest of "
               f"the FX complex (divergence +{div:.2f}σ over 5d). DXY-led regime.")
        msg_color = "#3b82f6"
    else:
        msg = (f"DXY is moving <b style='color:#fff;'>less</b> than the rest of "
               f"the FX complex (divergence {div:.2f}σ over 5d). EM FX or basis "
               f"is doing more work; the dollar headline understates the FX story.")
        msg_color = "#f97316"
...
# ---------------------------------------------------------------------------
# Funding stress side panel — JYBSS12M with stress thresholds
# ---------------------------------------------------------------------------
def _render_funding_stress_panel(prices: pd.DataFrame):
    info = funding_stress_summary(prices)
    if not info:
        return

    last = info["last"]
    last_5d = info["last_5d"]
    last_20d = info["last_20d"]
    stress_label = info["stress_label"]
    s = info["series"]

    stress_color_map = {
        "ACUTE STRESS":     "#f97316",
        "ELEVATED STRESS":  "#facc15",
        "MILD STRESS":      "#06b6d4",
        "NORMAL":           "#3b82f6",
        "USD ABUNDANT":     "#a855f7",
    }
    stress_color = stress_color_map.get(stress_label, "#888")
        ...
        st.markdown(
            ...
            <div style="text-align:left;">
              <div style="font-size:10px;color:#888;letter-spacing:0.08em;
                          text-transform:uppercase;">USDJPY 12M BASIS</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:24px;
                          font-weight:700;color:#fff;">{last:+.0f} bp</div>
              <div style="font-size:11px;color:{stress_color};font-weight:600;
                          letter-spacing:0.05em;">{stress_label}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            <div style="text-align:left;">
              ...
              <div style="font-family:'JetBrains Mono',monospace;font-size:18px;
                          font-weight:700;color:#fff;">{sign5}{last_5d:.0f} bp</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with cols[2]:
        st.markdown(
            ...
            <div style="text-align:left;">
              ...
              <div style="font-family:'JetBrains Mono',monospace;font-size:18px;
                          font-weight:700;color:#fff;">{sign20}{last_20d:.0f} bp</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with cols[3]:
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate="%{x|%Y-%m-%d}<br>basis: %{y:+.0f}bp<extra></extra>",
            showlegend=False,
        ))
        fig.add_hline(y=0, line=dict(color="rgba(255,255,255,0.2)", width=1))
        fig.add_hline(y=-50, line=dict(color="rgba(252,211,77,0.4)", width=1, dash="dot"),
                      annotation_text="elevated", annotation_position="left",
                      annotation_font=dict(color=TEXT_DIM, size=9))
        fig.add_hline(y=-100, line=dict(color="rgba(249,115,22,0.4)", width=1, dash="dot"),
                      annotation_text="acute", annotation_position="left",
                      annotation_font=dict(color=TEXT_DIM, size=9))
        fig.update_layout(
            **{**DARK_LAYOUT,
...
def _render_heatmap_panel(loadings: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
          ...
          3 rows, one per FX sub-component. Cell <b>brightness</b> = participation
          in the dominant theme. <b>+</b> = same direction as DXY (USD-strong),
          <b>−</b> = opposite. Note: when the basis row shows <b>−</b>, the dollar
          regime is NOT funding-driven — it's growth/rate-divergence driven.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.caption(
        "Top: how much variance the dominant FX theme is explaining. "
        "Bottom: concentration. 0.33 = diffuse (3 components equal); "
        "1.0 = pure single-component move."
    )
...
def _render_drilldown_panel(loadings: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
          ...
          <span style="color:#3b82f6;font-weight:600;">Blue</span> = same side as DXY,
        ...
        if asset == ANCHOR:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    hovertemplate=f"<b>{ASSET_LABELS[asset]}</b> (same side as DXY)"
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name=ASSET_LABELS[asset], showlegend=False,
                    hovertemplate=f"<b>{ASSET_LABELS[asset]}</b> (opposite DXY)"
                                  "<br>%{x|%Y-%m-%d}<br>|load| = %{y:.3f}<extra></extra>",
                ), row=i + 1, col=1)
    ...
    st.caption(
        ...
        "Blue fill = same side of PC1 as DXY · Orange fill = opposite."
    )
...
def _render_correlations_panel(returns: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
                    ...
                    margin-bottom:1rem;">
          <span style="color:#fbbf24;font-weight:600;">What this shows:</span>
          The 3 unique pairs in the FX complex. Cell <b>brightness</b> = |ρ|;
          signed value tells direction.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with cc1:
        window = st.slider(
            ...
            value=st.session_state.get("fx_corr_window", 20),
            key="fx_corr_window",
        )
    with cc2:
        weighting = st.radio(
            ...
            index=["equal", "ewm"].index(
                st.session_state.get("fx_corr_weighting", "equal")),
            format_func=lambda x: {"equal": "Equal", "ewm": "Exp (W/3)"}[x],
            key="fx_corr_weighting",
            horizontal=True,
        )
...
def _render_dominant_theme_panel(returns: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
                    ...
                    margin-bottom:1rem;">
          <span style="color:#fbbf24;font-weight:600;">What this shows:</span>
          PC1 loadings for all 3 FX sub-components, the leader, and concentration.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with pc1:
        window = st.slider(
            "Window (trading days)",
            min_value=5, max_value=252, step=1,
            value=st.session_state.get("fx_pca_window", 20),
            key="fx_pca_window",
        )
    with pc2:
        presmooth_halflife = st.select_slider(
            ...
            value=st.session_state.get("fx_pca_presmooth", 0),
            key="fx_pca_presmooth",
        )
    ...
    with pc3:
        weighting = st.radio(
            ...
            options=["equal", "ewm"],
            index=["equal", "ewm"].index(
                st.session_state.get("fx_pca_weighting", "equal")),
            format_func=lambda x: {"equal": "Equal", "ewm": "Exp (W/3)"}[x],
            key="fx_pca_weighting",
            horizontal=True,
        )
    with pc4:
        pca_method = st.radio(
            ...
            index=["standard", "procrustes"].index(
                st.session_state.get("fx_pca_method", "standard")),
            format_func=lambda x: {"standard": "Per-day DXY+",
    ...
    low_conf_mask = (roll["EigGap"] < 0.10) | (roll["ExplainedVar"] < 0.45)
...
def _render_regime_panel(loadings: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
                    ...
                    margin-bottom:1rem;">
          <span style="color:#fbbf24;font-weight:600;">What this shows:</span>
          Regime identified by <b>(leader, 3-bit sign pattern)</b>.
