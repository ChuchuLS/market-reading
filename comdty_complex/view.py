...
from commodities_complex.analytics import (
    ...
    headline_vs_breadth, gold_copper_ratio, oil_summary,
...
def render_commodities_complex():
    ...
    st.markdown(
        ...
        <div style="background:#0a0a0a;padding:1rem 1.25rem;margin:0 0 1rem 0;
          ...
          <div style="font-size:11px;color:#fbbf24;letter-spacing:0.2em;
            ...
            ⬢ Commodities · BCOM · WTI · Copper · Gold · Soybeans
          </div>
    ...
    with pc1:
        if st.button("⚡ Standard", use_container_width=True, key="cm_preset_std"):
            for ns in ("cm_corr", "cm_pca"):
                st.session_state[f"{ns}_window"] = 20
                st.session_state[f"{ns}_weighting"] = "equal"
            st.session_state["cm_scaling"] = "zscore"
            st.session_state["cm_pca_method"] = "standard"
            st.session_state["cm_pca_presmooth"] = 0
            st.rerun()
    with pc2:
        if st.button("〜 Smooth", use_container_width=True, key="cm_preset_smooth"):
            for ns in ("cm_corr", "cm_pca"):
                st.session_state[f"{ns}_window"] = 90
                st.session_state[f"{ns}_weighting"] = "equal"
            st.session_state["cm_scaling"] = "zscore"
            st.session_state["cm_pca_method"] = "procrustes"
            st.session_state["cm_pca_presmooth"] = 15
            st.rerun()
    with pc3:
        if st.button("≈ Very Smooth", use_container_width=True, key="cm_preset_vsmooth"):
            for ns in ("cm_corr", "cm_pca"):
                st.session_state[f"{ns}_window"] = 120
                st.session_state[f"{ns}_weighting"] = "ewm"
            st.session_state["cm_scaling"] = "zscore"
            st.session_state["cm_pca_method"] = "procrustes"
            st.session_state["cm_pca_presmooth"] = 20
            st.rerun()
    ...
    with tbc1:
        scaling = st.radio(
            ...
            index=["zscore", "volscale"].index(
                st.session_state.get("cm_scaling", "zscore")),
    ...
    with tbc2:
        st.markdown("<div style='font-size:10px;color:transparent;'>spacer</div>",
                    unsafe_allow_html=True)
        if st.button("↻ Refresh data", use_container_width=True, key="cm_refresh"):
            st.cache_data.clear()
            st.rerun()

    corr_window      = st.session_state.get("cm_corr_window", 20)
    corr_weighting   = st.session_state.get("cm_corr_weighting", "equal")
    pca_window       = st.session_state.get("cm_pca_window", 20)
    pca_weighting    = st.session_state.get("cm_pca_weighting", "equal")
    pca_method       = st.session_state.get("cm_pca_method", "standard")
    pca_presmooth    = st.session_state.get("cm_pca_presmooth", 0)
    ...
    settings_html = f"""
        <div style="background:#0f0f0f;border:1px solid #2a2a2a;border-left:3px solid #fbbf24;
          ...
          <div><span style="color:#fbbf24;font-weight:600;">SHARED:</span>
            scaling={scaling} · anchor=BCOM(+)</div>
        </div>
    """
    ...
    _render_gold_copper_oil_panel(prices)
...
def _render_headline_diagnostic(returns: pd.DataFrame):
    ...
    if abs(div) < 0.5:
        msg = "BCOM is moving in line with the rest of the commodity complex."
        msg_color = "#888"
    elif div > 0:
        msg = (f"BCOM is moving <b style='color:#fff;'>more</b> than the rest of "
               f"the complex (divergence +{div:.2f}σ over 5d). Broad commodity move.")
        msg_color = "#3b82f6"
    else:
        msg = (f"BCOM is moving <b style='color:#fff;'>less</b> than the rest of "
               f"the complex (divergence {div:.2f}σ over 5d). One sub-sector "
               f"(energy / metals / gold / ags) is doing more work; broad index masks the action.")
        msg_color = "#f97316"
...
def _render_gold_copper_oil_panel(prices: pd.DataFrame):
    ...
    if not gc or not oil:
        return

    regime_color_map = {
        "FEAR-ON":         "#facc15",
        "DEFENSIVE TILT":  "#06b6d4",
        "BALANCED":        "#3b82f6",
        "CYCLICAL TILT":   "#a855f7",
        "GROWTH-ON":       "#3b82f6",
    }
    gc_color = regime_color_map.get(gc["regime_label"], "#888")

    sign1m_gc = "+" if gc["pct_chg_1m"] >= 0 else ""
    sign3m_gc = "+" if gc["pct_chg_3m"] >= 0 else ""
    sign1m_oil = "+" if oil["pct_chg_1m"] >= 0 else ""
    sign3m_oil = "+" if oil["pct_chg_3m"] >= 0 else ""
        ...
        st.markdown(
            ...
            <div style="text-align:left;">
              <div style="font-size:10px;color:#888;letter-spacing:0.08em;
                          text-transform:uppercase;">GOLD/COPPER</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:24px;
                          font-weight:700;color:#fff;">{gc['last']:.1f}</div>
              <div style="font-size:11px;color:{gc_color};font-weight:600;
                          letter-spacing:0.05em;">{gc['regime_label']}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            <div style="text-align:left;">
              ...
              <div style="font-family:'JetBrains Mono',monospace;font-size:18px;
                          font-weight:700;color:#fff;">{sign1m_gc}{gc['pct_chg_1m']:.1f}%</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with cols[2]:
        st.markdown(
            ...
            <div style="text-align:left;">
              ...
              <div style="font-family:'JetBrains Mono',monospace;font-size:18px;
                          font-weight:700;color:#fff;">{sign3m_gc}{gc['pct_chg_3m']:.1f}%</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with cols[3]:
        st.markdown(
            ...
            <div style="text-align:left;">
              <div style="font-size:10px;color:#888;letter-spacing:0.08em;
                          text-transform:uppercase;">WTI CRUDE</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:24px;
                          font-weight:700;color:#fff;">${oil['last']:.1f}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            <div style="text-align:left;">
              <div style="font-size:10px;color:#888;letter-spacing:0.08em;
                          text-transform:uppercase;">1M Δ</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:18px;
                          font-weight:700;color:#fff;">{sign1m_oil}{oil['pct_chg_1m']:.1f}%</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            <div style="text-align:left;">
              <div style="font-size:10px;color:#888;letter-spacing:0.08em;
                          text-transform:uppercase;">3M Δ</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:18px;
                          font-weight:700;color:#fff;">{sign3m_oil}{oil['pct_chg_3m']:.1f}%</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def _render_heatmap_panel(loadings: pd.DataFrame):
    ...
    fig_aux.add_hline(y=0.20, line=dict(color="rgba(255,255,255,0.25)",
                                        dash="dot", width=1), row=2, col=1,
                      annotation_text="diffuse floor (0.20)",
                      annotation_position="left",
                      annotation_font=dict(color=TEXT_DIM, size=9))
    fig_aux.add_hline(y=0.50, line=dict(color="rgba(255,255,255,0.15)",
                                        dash="dot", width=1), row=2, col=1)
...
def _render_drilldown_panel(loadings: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
          ...
          <span style="color:#3b82f6;font-weight:600;">Blue</span> = same side as BCOM,
        ...
        if asset == ANCHOR:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    hovertemplate=f"<b>{ASSET_LABELS[asset]}</b> (same side as BCOM)"
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name=ASSET_LABELS[asset], showlegend=False,
                    hovertemplate=f"<b>{ASSET_LABELS[asset]}</b> (opposite BCOM)"
                                  "<br>%{x|%Y-%m-%d}<br>|load| = %{y:.3f}<extra></extra>",
                ), row=i + 1, col=1)
...
def _render_correlations_panel(returns: pd.DataFrame):
    ...
    with cc1:
        window = st.slider(
            ...
            value=st.session_state.get("cm_corr_window", 20),
            key="cm_corr_window",
        )
    with cc2:
        weighting = st.radio(
            ...
            index=["equal", "ewm"].index(
                st.session_state.get("cm_corr_weighting", "equal")),
            format_func=lambda x: {"equal": "Equal", "ewm": "Exp (W/3)"}[x],
            key="cm_corr_weighting",
            horizontal=True,
        )
    ...
    PAIR_PALETTE = [
        "#3b82f6", "#06b6d4", "#f97316", "#a855f7", "#facc15",
        "#0ea5e9", "#fb923c", "#7c3aed", "#0891b2", "#fcd34d",
    ]
...
def _render_dominant_theme_panel(returns: pd.DataFrame):
    ...
    with pc1:
        window = st.slider(
            "Window (trading days)",
            min_value=5, max_value=252, step=1,
            value=st.session_state.get("cm_pca_window", 20),
            key="cm_pca_window",
        )
    with pc2:
        presmooth_halflife = st.select_slider(
            ...
            value=st.session_state.get("cm_pca_presmooth", 0),
            key="cm_pca_presmooth",
        )
    ...
    with pc3:
        weighting = st.radio(
            ...
            options=["equal", "ewm"],
            index=["equal", "ewm"].index(
                st.session_state.get("cm_pca_weighting", "equal")),
            format_func=lambda x: {"equal": "Equal", "ewm": "Exp (W/3)"}[x],
            key="cm_pca_weighting",
            horizontal=True,
        )
    with pc4:
        pca_method = st.radio(
            ...
            index=["standard", "procrustes"].index(
                st.session_state.get("cm_pca_method", "standard")),
            format_func=lambda x: {"standard": "Per-day BCOM+",
...
def _render_regime_panel(loadings: pd.DataFrame):
    st.markdown(
        ...
        <div style="background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.2);
                    ...
                    margin-bottom:1rem;">
          <span style="color:#fbbf24;font-weight:600;">What this shows:</span>
          Regime by <b>(leader, 5-bit sign pattern)</b>.
