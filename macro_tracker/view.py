...
from theming import (
    BG, GRID, TEXT, TEXT_DIM, GREEN, RED, AMBER, BLUE,
    CATEGORY_COLORS, DARK_LAYOUT, COLS,
)
...
def render_macro_tracker():
    ...
    if not DATA_PATH.exists():
        st.error(f"DATA.xlsx not found at {DATA_PATH}. "
    ...
    with st.sidebar:
        ...
        cat_filter = st.selectbox("Category", all_cats, index=0,
        ...
        if st.button("↻ Refresh BQL data", use_container_width=True,
                     key="mt_refresh"):
            st.cache_data.clear()
            st.rerun()
    ...
    with c2:
        st.metric("YTD Breadth", f"{positive} / {negative}",
                  f"{breadth_pct:.0f}% positive", delta_color="off")
    with c3:
        st.metric("YTD Leader", leader_row["Name"], f"+{leader_row['YTD']:.2f}%")
    with c4:
        st.metric("YTD Laggard", laggard_row["Name"],
                  f"{laggard_row['YTD']:.2f}%", delta_color="inverse")
...
def _render_cross_timeframe(df: pd.DataFrame):
    ...
    def _build_fig(scatter_df: pd.DataFrame, header: str, n_total: int) -> go.Figure:
        ...
        if len(scatter_df) == 0:
            fig.add_annotation(
                ...
                text="No instruments in this group", showarrow=False,
        ...
        for cat in sorted(scatter_df["Category"].unique()):
            ...
            fig.add_trace(go.Scatter(
                x=sub[x_axis], y=sub[y_axis], mode="markers+text", name=cat,
        ...
        fig.add_annotation(
            x=x_range[1], y=y_range[1] + y_pad * 0.3,
        ...
        fig.add_annotation(
            x=x_range[0], y=y_range[0] - y_pad * 0.3,
        ...
        fig.update_layout(
            **DARK_LAYOUT, height=520, showlegend=True,
            title=dict(
    ...
    fig_etf = _build_fig(etfs, "EQUITIES & ETFS",
                         (df["InstrumentType"] == "ETF").sum())
    st.plotly_chart(fig_etf, use_container_width=True, config={"displayModeBar": False})

    fig_com = _build_fig(comdty, "COMMODITY FUTURES",
                         (df["InstrumentType"] == "Comdty").sum())
