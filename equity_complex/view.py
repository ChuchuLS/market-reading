...
def render_equity_complex():
    ...
    with pc3:
        if st.button(
            "≈ Very Smooth", use_container_width=True, key="eq_preset_vsmooth"
        ):
            for ns in ("eq_corr", "eq_pca"):
...
def _render_headline_diagnostic(returns: pd.DataFrame):
    ...
    if abs(div) < 0.5:
        ...
        msg = (
            f"SPX is moving <b style='color:#fff;'>more</b> than the rest of "
            f"the complex (divergence +{div:.2f}σ over 5d). Cap-weighted leadership."
        )
        msg_color = "#3b82f6"
    else:
        msg = (
            f"SPX is moving <b style='color:#fff;'>less</b> than the rest of "
